...
function updateSelectedAccount(authUser){
    ...
    if(authUser != null){
        ...
        if(authUser.uuid != null){
            document.getElementById('avatarContainer').style.backgroundImage = `url('https://crafatar.com/renders/head/${authUser.uuid}?overlay')`
        }
    }
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    ...
                    'Pour rejoindre GuildHorizon, vous avez besoin d\'une installation 64 bits de Java 8. Souhaitez-vous que nous installions une copie ? En installant, vous acceptez <a href="http://www.oracle.com/technetwork/java/javase/terms/license/index.html">Oracle\'s license agreement</a>.',
                    'Installer Java',
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            'Java est requis<br>pour lancer le jeu',
                            'Une installation x64 valide de Java 8 est requise pour démarrer.',
                            'Je comprends',
                            'Fermer'
                        )
                        setOverlayHandler(() => {
            ...
            if(m.result === true){
                ...
                setOverlayContent(
                    'Problème inattendu:<br>Le téléchargement de Java a échoué',
                    'Malheureusement, nous avons rencontré un problème lors de la tentative d\'installation de Java. Vous devrez installer manuellement une copie.',
                    'Je comprends'
                )
                setOverlayHandler(() => {
                    toggleOverlay(false)
        ...
        if(m.context === 'validate'){
            switch(m.data){
                ...
                case 'version':
                    ...
                    setLaunchDetails('Valider l\'intégrité des ressources..')
                    break
                case 'assets':
                    setLaunchPercentage(60, 100)
                    loggerLaunchSuite.log('Asset Validation Complete')
                    setLaunchDetails('Validation de l\'intégrité des bibliothèques..')
                    break
                case 'libraries':
            ...
            if(login && allGood) {
                ...
                const gameStateChange = function(data){
                    ...
                    if(SERVER_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Sur le champ de bataille !')
                    } else if(GAME_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Arrive à Alena !')
                    }
                }
                ...
                const gameErrorListener = function(data){
                    ...
                    if(data.indexOf('Could not find or load main class net.minecraft.launchwrapper.Launch') > -1){
                        ...
                        showLaunchFailure('Erreur lors du lancement', 'Le fichier principal, LaunchWrapper, n\'a pas pu être téléchargé correctement. En conséquence, le jeu ne peut pas se lancer.<br><br>Pour résoudre ce problème, désactivez temporairement votre logiciel antivirus et relancez le jeu.')
                    }
                }
...
function setNewsLoading(val){
    if(val){
        const nLStr = 'Actualisation des actualités..'
        let dotStr = '..'
